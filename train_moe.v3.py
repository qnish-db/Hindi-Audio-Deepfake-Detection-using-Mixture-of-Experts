# train_moe.v4.py — Architectural improvements + revert to working hyperparameters
...
# -------------------------
# Defaults (REVERTED to v3 working hyperparameters)
# -------------------------
...
DEFAULTS = dict(
    ...
    num_workers=0,  # FIX: Set to 0 to avoid Windows paging file error

    # REVERTED: Back to working hyperparameters from v3
    expert_bottleneck=768,      # was 384 in Tier-1
    expert_dropout=0.2,         # was 0.4 in Tier-1

    gate_hidden=64,
    gate_dropout=0.1,

    lr=1e-3,
    weight_decay=1e-2,          # was 5e-2 in Tier-1
    betas=(0.9, 0.999),
    warmup_ratio=0.05,
    cosine_final_lr=1e-4,
    max_epochs=30,
    patience=5,
    label_smoothing=0.05,       # was 0.15 in Tier-1
    aux_loss_lambda=0.2,
    gate_balance_lambda=0.1,
    grad_clip=1.0,
    amp=True,
    ckpt_dir="checkpoints",
    run_name="moe_ptm2_v4_arch_fix",
    quiet=False,
    ...
    use_batchnorm=True,         # Use BatchNorm instead of LayerNorm
    use_se=False,               # Disable SE module (too specific)
    simple_gate=True,           # Simplified gate network
    stochastic_depth=0.8,       # Expert dropout during training (0.8 = 80% keep prob)
    use_fusion=True,            # Add fusion layer after MoE
)
...
class SE1D(nn.Module):
    """Channel attention - kept as optional"""
...
class ImprovedExpert(nn.Module):
    def __init__(self, in_dim=1536, bottleneck=768, drop=0.2, use_batchnorm=True, use_se=False):
        super().__init__()
        self.use_se = use_se
        
        # CHANGE 1: BatchNorm instead of LayerNorm for better generalization
        if use_batchnorm:
        ...
        # CHANGE 2: Optional SE (disabled by default)
        if use_se:
...
class TinyGate(nn.Module):
    def __init__(self, in_dim_concat, hidden=64, drop=0.1, n_experts=2, simple=False):
        super().__init__()
        # CHANGE 3: Simpler gate to reduce overfitting
        if simple:
...
class MoEModel(nn.Module):
    def __init__(self, ptms: List[str], in_dim_each=1536, expert_bottleneck=768, expert_drop=0.2,
                 gate_hidden=64, gate_drop=0.1, use_batchnorm=True, use_se=False, 
                 simple_gate=False, stochastic_depth=1.0, use_fusion=False):
        super().__init__()
        self.ptms = ptms
        ...
        # CHANGE 4: Add fusion layer for better combination
        if use_fusion:
    ...
    def forward(self, x_dict: Dict[str, torch.Tensor]):
        ...
        # CHANGE 5: Stochastic depth for experts during training
...
def train_loop(cfg):
    ...
    print(f"\n[architecture] BatchNorm={cfg['use_batchnorm']} | SE={cfg['use_se']} | SimpleGate={cfg['simple_gate']} | StochasticDepth={cfg['stochastic_depth']} | Fusion={cfg['use_fusion']}")
