# ===== Wav2Vec2-Base — subset runner (resumable, with progress bars & safe Ctrl+C) =====
...
SUBSET_V2  = META_DIR / "ptm_subset.v2.csv"   # contains: all fake + 50% real
...
def sanity():
    ...
    if not SUBSET_V2.exists(): raise FileNotFoundError(f"Missing subset file: {SUBSET_V2}")
...
# ---------- Load subset + (re)build TODO (resumable) ----------
subset = pd.read_csv(SUBSET_V2)
...
m   = AutoModel.from_pretrained(
        ...
        use_safetensors=True   # <— prefer .safetensors and avoid .bin load
    ).to(DEVICE).eval()
