# train_moe.v6.py — MAXIMUM regularization push for 93-95%
...
DEFAULTS = dict(
    ...
    expert_dropout=0.4,         # ↑ from 0.3

    gate_hidden=64,
    gate_dropout=0.2,           # ↑ from 0.15

    lr=6e-4,                    # ↓ even lower
    weight_decay=3e-2,          # ↑ even stronger
    betas=(0.9, 0.999),
    warmup_ratio=0.08,          # ↑ longer warmup
    cosine_final_lr=5e-6,       # ↓ very low final
    max_epochs=40,              # ↑ more epochs
    patience=10,                # ↑ more patience
    label_smoothing=0.15,       # ↑ from 0.1
    aux_loss_lambda=0.1,        # ↓ less aux weight
    gate_balance_lambda=0.2,    # ↑ stronger balance
    grad_clip=0.3,              # ↓ very tight
    amp=True,
    ckpt_dir="checkpoints",
    run_name="moe_ptm2_v6_maximum",
    quiet=False,

    calibrate=True,
    calib_frac=0.2,
    
    # MAXIMUM architectural controls
    use_batchnorm=True,
    use_se=False,
    simple_gate=True,
    stochastic_depth=0.5,       # ↓ 50% expert dropout (from 0.6)
    use_fusion=True,
    fusion_dropout=0.6,         # ↑ even heavier (from 0.5)
    
    # Stronger Mixup
    mixup_alpha=0.4,            # ↑ from 0.3
    mixup_prob=0.6,             # ↑ from 0.5 (60% of batches)
    
    # NEW: Feature noise injection
    feature_noise=0.02,         # Add Gaussian noise to features
)
...
# NEW: Feature noise injection
def add_feature_noise(x_dict: Dict[str, torch.Tensor], noise_std: float, training: bool):
...
class ImprovedExpert(nn.Module):
    def __init__(self, in_dim=1536, bottleneck=768, drop=0.4, use_batchnorm=True, use_se=False):
        super().__init__()
        self.use_se = use_se
...
class TinyGate(nn.Module):
    def __init__(self, in_dim_concat, hidden=64, drop=0.2, n_experts=2, simple=False):
        super().__init__()
        if simple:
...
class MoEModel(nn.Module):
    def __init__(self, ptms: List[str], in_dim_each=1536, expert_bottleneck=768, expert_drop=0.4,
                 gate_hidden=64, gate_drop=0.2, use_batchnorm=True, use_se=False, 
                 simple_gate=False, stochastic_depth=0.5, use_fusion=False, fusion_dropout=0.6):
        super().__init__()
        self.ptms = ptms
...
def train_loop(cfg):
    ...
    print(f"\n[v6 MAXIMUM] Mixup={cfg['mixup_alpha']:.2f}@{cfg['mixup_prob']:.0%} | StochDepth={cfg['stochastic_depth']} | FusionDrop={cfg['fusion_dropout']} | FeatureNoise={cfg['feature_noise']} | LabelSmooth={cfg['label_smoothing']}")
    ...
    for epoch in range(1, cfg["max_epochs"] + 1):
        ...
        for batch in pbar:
            ...
            xs = add_feature_noise(xs, cfg["feature_noise"], model.training)
...
def parse_args():
    ...
    ap.add_argument("--feature-noise", type=float, default=DEFAULTS["feature_noise"])
